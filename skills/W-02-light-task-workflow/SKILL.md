...
Light-task-workflow maintains one task file under `tasks/`.
...
2. search `tasks/` for an existing active task covering the same scope before creating a new one
...
9. after approval, treat code changes, onboarding propagation through `C-05-create-or-update-onboarding-files`, and the checks listed in `docs/tools.md` as one implementation cycle
...
1. `docs/` for relevant reference material when it exists
2. `docs/glossary/` for canonical terminology when it exists
3. onboarding for any repo whose behavior the artifact touches
...
7. After approval, onboarding is updated through `C-05-create-or-update-onboarding-files` and the listed checks in `docs/tools.md` are run.
