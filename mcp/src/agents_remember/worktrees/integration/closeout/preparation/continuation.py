    ...
    def finalize(self, handoff: CloseoutCertificationHandoff) -> WorktreeCommandResult:
        ...
        if len(current.selected.terminals) != 5:
            refuse(
                ...
                "selected fifth certificate",
                current.selected.state,
            )
        terminal = current.selected.terminals[-1]
        if (
            terminal.certificate is None
            or terminal.certificateReference is None
            or terminal.certificate.semanticEnvelope.gateFiveInputs is None
        ):
            refuse(
                "finalization-memory-certificate-missing",
                "original green fifth certificate",
                terminal.resultReference,
            )
        result = PreparedMemoryCertificationResult(
            current,
            candidate,
            terminal.certificate.semanticEnvelope.gateFiveInputs,
            terminal.resultReference,
