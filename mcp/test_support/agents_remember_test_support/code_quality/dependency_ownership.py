...
AMBIENT_ROLE_SELECTION_PATH = Path("scripts") / "e2e_harness" / "selection.py"
...
REPOSITORY_TEST_INPUT_CONSUMERS: dict[Path, frozenset[Path]] = {
    AMBIENT_ROLE_SELECTION_PATH: frozenset(
        {
...
class DependencyOwnershipGraph:
    ...
    def _repository_consumers(
        ...
        declared = (
            repository_declared
            if repository_declared is not None
            else self.declared_consumers.get(path)
        )
        ...
        if declared is not None:
            ...
            owned = self._owned(
                ...
                SelectionReasonKind.DECLARED_CONSUMER,
                path,
                "verified-repository-input"
                if repository_declared is not None
                else "verified-catalog",
            )
