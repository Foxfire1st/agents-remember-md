...
"Capture what code can't say on its own" is a developer-facing statement of this rule. The work-onboarding frame makes it intuitive: *write the onboarding the way a good senior would explain this codebase to a new hire — the things that aren't obvious from reading the code.* That is the scope.
...
`agents-remember.md` inverts this dynamic in one structural way: the onboarding layer is where work happens, not alongside it. An engineer fixing a bug reads onboarding alongside code, makes the fix, updates the onboarding if the fix revealed something durable. A senior correcting a junior's approach corrects it in the onboarding. There is no separate documentation task — the approval gate, the drift check, and the post-implementation update are the work flow itself. Contribution is not a tax on work; it is embedded in how the work gets done.

This creates a cascade. When a senior corrects an onboarding, the next engineer who touches that area — junior human, mid-level human, AI agent, does not matter — reads the corrected onboarding and makes a better decision than they would have. Their better decision goes into the onboarding too. The system gets smoother with use.
