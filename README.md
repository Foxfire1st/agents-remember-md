...
- **A phased task structure that enforces process instead of hoping for it.** Seven phases — Creation, Research, Synthesis, Design, Plan, Implementation, Closing — with review gates between each. LLMs have no meta-awareness of skipped workflow steps; the phase structure itself has to enforce sequencing, because advisory skills and tracking files will not.
...
The input layer pulls the relevant subset of onboarding files into the task scope and annotates each one with the normalized requirement IDs that may touch it, based on the requirements. The same input documentation fixes which existing features and functionality must be preserved, so nothing gets silently dropped during the work. That present-state record is grounded on the input project documentation + code — so what must survive the change is itself verifiable rather than relying on the developer's memory of what the system does. A dedicated Synthesis phase then turns the remaining unknowns into two parallel question sets — one about what should be true, one about how to do it — before Design tries to answer either.
...
- **Intent** — the reasoning that justifies the projected code. Why this shape rather than another. How the change fits the larger system. Which invariants are being preserved, broken, or newly established. Which preserved-behavior items captured in the input documentation are being addressed, and how.
...
- **Research is grounded in reality, not intent.** The Research phase builds task-local input documentation — what changes, what moves, what stays — by reading onboarding files and code together, with normalized requirement IDs annotated against the present-state surfaces they touch. This catches assumptions that pure-requirements workflows miss because they never model the existing system.
...
- **Context survives the LLM.** Onboarding files are durable. They outlive context window compression, session boundaries, and model swaps. The agent can recover full task state by reading a small set of root contracts (`task.md`, `requirements.md`, `architecture.md`) plus the active phase's `progress.md`. Nothing important lives only in a conversation.
